...
async function removeCreatedAtFields() {
  console.log('🚀 DynamoDBからcreatedAtフィールドを削除開始...');
  ...
  try {
    ...
    do {
      ...
      // createdAtフィールドを持つレコードをフィルタ
      const recordsWithCreatedAt = scanResult.Items.filter(item => item.createdAt);
      console.log(`🎯 createdAtフィールドを持つレコード: ${recordsWithCreatedAt.length}件`);
      
      // 各レコードからcreatedAtを削除
      for (const item of recordsWithCreatedAt) {
        try {
          ...
          const updateParams = {
            ...
            ConditionExpression: 'attribute_exists(createdAt)', // createdAtが存在する場合のみ実行
            ReturnValues: 'NONE'
          };
          ...
          if (error.name === 'ConditionalCheckFailedException') {
            console.log(`⚠️  スキップ (createdAtなし): ${item.userId}/${item.memoId}`);
          } else {
...
// 実行確認
console.log('⚠️  この操作はDynamoDBからcreatedAtフィールドを完全に削除します');
console.log('📍 対象テーブル:', tableName);
console.log('🔄 5秒後に開始します... (Ctrl+Cで中止)');

setTimeout(() => {
  removeCreatedAtFields();
}, 5000);